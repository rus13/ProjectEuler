...
def maxGrid(grid):
    ...
    for i in range(rows-3):
        ...
        for j in range(columns):
            ...
            if j >= 3:
                dleft = grid[i][j] * grid[i+1][j-1] * grid[i+2][j-2] * grid[i+3][j-3]
            if columns - j >= 4:
                right = grid[i][j] * grid[i][j+1] * grid[i][j+2] * grid[i][j+3]
                dright = grid[i][j] * grid[i+1][j+1] * grid[i+2][j+2] * grid[i+3][j+3]
            maxproduct = max(maxproduct, dleft, down, dright, right)
    return maxproduct
